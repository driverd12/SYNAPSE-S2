...
| Retrieval v2 measurement harness | Verified synthetic regression gate | `scripts/measure_retrieval_v2.py`, `tests/fixtures/retrieval_v2/benchmark_v1.json`, and Retrieval v2 measurement tests check Recall@k, MRR, nDCG@k, namespace leakage, duplicates, score/provenance contracts, deterministic output across insertion/backend variations, and read purity in disposable stores. `docs/evidence/phase8-retrieval-v2-acceptance.json` is bound to clean implementation commit `738cfce` and passes every gate. Passing is not proof of live-corpus relevance or production latency. |
