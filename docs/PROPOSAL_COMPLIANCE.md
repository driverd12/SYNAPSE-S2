...
| Hardened capture and pruning safety envelope | Implemented | `redaction.py`, pre-write capture inbox redaction, private local file modes, symlink rejection, direct capture/context-bus response redaction, MCP `confirm=true` pruning, CLI `--confirm`, and tests |
...
| Destructive prune tool calls | CLI, MCP, and dashboard destructive graph pruning are confirmation-gated | Operators need fast surgical cleanup while preventing accidental node, edge, temporal, or associative deletion. |
