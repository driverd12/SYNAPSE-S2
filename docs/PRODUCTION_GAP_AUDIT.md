...
10. Text memories show `embedding_provider` provenance from the active local provider, event memories show `surprise_model`, `surprise_mode`, `semantic_surprise_score`, and `lexical_surprise_score`, and `provider-benchmark`/`benchmark_spiking_embedding_provider` reports `mlx-neural-v1` plus `native_mlx: true` for installed client defaults.
...
12. `enter-cortex`, `cortex-tick`, `commit-cortex`, and `cortex-state` or their MCP equivalents can start a governed session, warn on missing scoped file/tool intent for mutations, warn before risky actions, and persist typed validation/decision/constraint/risk traces.
13. App Connect can list local apps, attach a confirmed app, capture selected text or a redacted app snapshot, and persist those captures as real temporal graph memory.
14. The dashboard at `http://127.0.0.1:8765/?context_id=default` can write, capture conversations, attach and snapshot local apps, process magic capture drops, ingest, enter/tick/commit Cortex Governor sessions with scoped intended files/tools, promote/demote/prune governed traces, recall, graph, prune, certify native runtime, sleep, back up, and show context-bus receipt state.
