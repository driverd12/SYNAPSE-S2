...
12. `enter-cortex`, `cortex-tick`, `commit-cortex`, and `cortex-state` or their MCP equivalents can start a governed session, warn before mutations, and persist typed validation/decision/constraint/risk traces.
13. The dashboard at `http://127.0.0.1:8765/?context_id=default` can write, capture conversations, process magic capture drops, ingest, enter/tick/commit Cortex Governor sessions, promote/demote/prune governed traces, recall, graph, prune, certify native runtime, sleep, back up, and show context-bus receipt state.
