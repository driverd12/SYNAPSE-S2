...
COMMAND="cd '$ROOT' || exit 2; export MLX_DEVICE='${MLX_DEVICE:-gpu}' SYNAPSE_S2_CAPTURE_ROOT='$CAPTURE_ROOT' SYNAPSE_S2_STATE_PATH='$STATE_PATH' SYNAPSE_S2_MEMORY_DB='$MEMORY_DB'; exec '$PYTHON' capture_daemon.py --capture-root '$CAPTURE_ROOT' --state '$STATE_PATH' --memory-db '$MEMORY_DB' --poll-interval '$POLL_INTERVAL'"
