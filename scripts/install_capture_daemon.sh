...
COMMAND="cd '$ROOT' || exit 2; export MLX_DEVICE='${MLX_DEVICE:-gpu}' SYNAPSE_S2_EMBEDDING_PROVIDER='$EMBEDDING_PROVIDER' SYNAPSE_S2_NEURAL_MODEL='$NEURAL_MODEL' SYNAPSE_S2_NEURAL_CACHE_DIR='$NEURAL_CACHE_DIR' SYNAPSE_S2_CAPTURE_ROOT='$CAPTURE_ROOT' SYNAPSE_S2_STATE_PATH='$STATE_PATH' SYNAPSE_S2_MEMORY_DB='$MEMORY_DB' SYNAPSE_S2_TRANSCRIPT_POLL='$TRANSCRIPT_POLL'; exec '$PYTHON' capture_daemon.py --capture-root '$CAPTURE_ROOT' --state '$STATE_PATH' --memory-db '$MEMORY_DB' --poll-interval '$POLL_INTERVAL'$TRANSCRIPT_ARGS"
